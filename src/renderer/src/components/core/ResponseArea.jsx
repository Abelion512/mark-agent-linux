            ...
            <HoloCard title="Detail Informasi" defaultExpanded={false}>
              ...
              {sources && sources.length > 0 && (
                <div className="mt-4 pt-4 border-t border-[var(--glass-border)]">
                  <h4 className="text-xs font-semibold uppercase text-primary mb-2 opacity-70">Sumber Referensi</h4>
                  <div className="flex flex-col gap-2">
                    {sources.map((src, i) => (
                      <a key={i} href={src.url} target="_blank" rel="noreferrer" className="text-sm text-info hover:underline opacity-80 hover:opacity-100 truncate block">
                        {src.title || src.url}
                      </a>
                    ))}
                  </div>
              ...
              {youtubeData && (
                <div className="mt-4">
                  <YoutubeSearchBubble queryYoutube={youtubeData.query} youtubeLink={youtubeData.link} />
                </div>
              )}
              {youtubeSummary && (
                <div className="mt-4">
