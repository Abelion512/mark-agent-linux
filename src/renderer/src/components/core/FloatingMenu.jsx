...
const FloatingMenu = ({ onOpenHistory, waStatus = 'disconnected' }) => {
  ...
  return (
    <div className="fixed top-8 left-8 z-50" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute top-16 left-0 w-64 glass glass-hover border border-[var(--glass-border)] rounded-2xl p-2 flex flex-col gap-1 shadow-[0_8px_32px_rgba(0,0,0,0.5)] animate-[holo-enter_0.2s_ease-out_forwards]">
          <button
            onClick={() => handleNav('/config')}
            className="flex items-center gap-3 w-full p-3 rounded-xl glass glass-hover transition-colors text-white/80 hover:text-white text-sm font-medium text-left"
          >
          ...
          <button
            ...
            className="flex items-center gap-3 w-full p-3 rounded-xl glass glass-hover cursor-pointer text-white/80 text-sm font-medium"
          >
