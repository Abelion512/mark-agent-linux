...
const HistoryDrawer = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 glass glass-hover z-[60] animate-[response-fade-in_0.2s_ease-out_forwards]"
          onClick={onClose}
        />
      )}
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-[var(--glass-border)]">
          <h2 className="text-xl font-semibold flex items-center gap-2">
            <MessageCircle className="text-white/70" />
            Riwayat Obrolan
          </h2>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full glass glass-hover hover:text-error hover:border-error transition-colors"
          >
            <X />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar flex flex-col gap-2">
          ...
          {historyTurns.length === 0 && (
            <div className="text-center text-white/50 mt-10">Belum ada riwayat.</div>
          )}
        </div>
      </div>
      ...
      {isOpen && previewData && (
        <div className="fixed top-1/2 left-[calc(50%-12rem)] -translate-x-1/2 -translate-y-1/2 w-full max-w-3xl z-[65] animate-[response-fade-in_0.3s_ease-out_forwards] pointer-events-none">
          <div className="glass glass-hover border border-[var(--glass-border)] rounded-3xl p-8 max-h-[80vh] overflow-y-auto custom-scrollbar shadow-[0_0_50px_rgba(0,0,0,0.5)] pointer-events-auto">
            <div className="flex justify-between items-center mb-6 sticky top-0 glass glass-hover pb-4 border-b border-white/5 z-20">
              ...
              <button
                ...
                className="text-white/50 hover:text-error transition-colors"
              >
                <X size={20} />
              </button>
            </div>
