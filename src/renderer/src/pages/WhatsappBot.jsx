...
import { MessageCircle, QrCode, Plug, LogOut, ArrowLeft } from 'lucide-react'
    ...
    <div className="relative w-full h-full flex flex-col glass">
      ...
      <div className="navbar glass glass-hover border-b border-white/5 sticky top-0 z-0">
        <div className="flex-1">
          <div className="flex items-center gap-3 px-4">
            ...
            <div className="avatar">
              <div className="w-10 rounded-full bg-white/15 p-2 text-white/70 flex items-center justify-center">
        ...
        <div className="flex-none gap-2 px-4">
          {status === 'disconnected' && (
            <button onClick={startBot} className="btn btn-sm btn-white/15">
              <Plug /> Connect
            </button>
          )}
          {status === 'connected' && (
      ...
      <div className="flex-1 overflow-y-auto w-full max-w-4xl mx-auto p-4 flex flex-col gap-4">
        {status === 'qr' && qrCode && (
          <div className="flex-1 flex flex-col items-center justify-center animate-fade-in">
            <div className="card glass glass-hover shadow-xl border border-white/10 max-w-sm w-full">
              <div className="card-body items-center text-center">
                <h2 className="card-title text-white/70 mb-2">
                  <QrCode /> Scan QR Code
                </h2>
                <div className="bg-white p-4 rounded-2xl shadow-lg mb-4 glass glass-hover bg-white/5 border border-white/10">
        ...
        {status === 'connected' && messages.length === 0 && (
          <div className="flex-1 flex flex-col items-center justify-center opacity-40 select-none">
            <MessageCircle className="text-6xl mb-4" />
        ...
        {messages.map((msg, idx) => (
          ...
          >
            ...
            <div
              className={`chat-bubble flex flex-col gap-1 ${msg.type === 'outgoing' ? 'chat-bubble-success text-success-content' : 'glass glass-hover'}`}
            >
              {msg.quotedText && (
                <div className="glass glass-hover mt-1 mb-2 rounded-xl border-l-4 border-white/30 px-2 py-1 rounded text-[10px] italic max-w-[250px] truncate opacity-80">
                  {msg.quotedText}
                </div>
              )}
        ...
        {isThinking && (
          <div className="chat chat-end animate-fade-in">
            ...
            <div className="chat-bubble chat-bubble-success bg-success/20 text-success border border-success/30 glass glass-hover">
